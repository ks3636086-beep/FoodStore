...
public partial class auth_manage_product : System.Web.UI.Page
{
    ...
    void PopulateGridview()
    {
        ...
        cmd.CommandText = "SELECT id,product_id,verticle_name,product_full_name,product_hsnORsac,product_parent_category_name,publish_status,product_sub_category_name,product_postion_no,(select top 1 product_stock from ecommerce_product_price where product_id=ecommerce_product.product_id) as product_stock FROM ecommerce_product where product_seller_id='0' AND (product_short_name LIKE '%' + @search + '%' OR product_full_name LIKE '%' + @search + '%' OR product_description LIKE '%' + @search + '%' OR product_hsnORsac LIKE '%' + @search + '%' OR product_parent_category_name LIKE '%' + @search + '%' OR product_sub_category_name LIKE '%' + @search + '%' OR product_brand_name LIKE '%' + @search + '%' OR product_full_description LIKE '%' + @search + '%') order by id asc";
        cmd.Connection = con;
    ...
    protected void btndelete_ServerClick1(object sender, EventArgs e)
    {
        ...
        foreach (GridViewRow row in grdproducts.Rows)
        {
            if ((row.FindControl("chk_delete") as CheckBox).Checked)
            {
                ...
                string update_status = "update ecommerce_product set top_deal=@top_deal where product_id=@product_id";
                SqlCommand cmd_status = new SqlCommand(update_status, con);

                cmd_status.Parameters.AddWithValue("@top_deal", "True");
                cmd_status.Parameters.AddWithValue("@product_id", (row.FindControl("lblproductid") as Label).Text);

                int success = cmd_status.ExecuteNonQuery();

                if (success > 0)
                {
                    ShowMessage("Add in top selling prodcuts.", MessageType.Success);
                }
    ...
    protected void btndelete_ServerClick2(object sender, EventArgs e)
    {
        int i = 0;
        ...
        string update_status = "update ecommerce_product set featured=@featured where product_id=@product_id";
        SqlCommand cmd_status = new SqlCommand(update_status, con);

        cmd_status.Parameters.AddWithValue("@featured", "True");
        cmd_status.Parameters.AddWithValue("@product_id", (row.FindControl("lblproductid") as Label).Text);
        ...
        if (success > 0)
        {
            ShowMessage("Add in featured prodcuts.", MessageType.Success);
        }
